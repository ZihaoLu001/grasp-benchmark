...
class PublicDocumentationDefaultsTest(unittest.TestCase):
    ...
    def test_readme_uses_current_public_language(self) -> None:
        ...
        self.assertIn("A target-specific Contact-GraspNet H100 rerun records `0 / 24`", text)
        self.assertIn("all 24 trials localized a target and produced a non-empty mask", text)
        ...
        self.assertIn("Contact-GraspNet Modular Pipeline", text)
        self.assertIn("target-specific trials", text)
        self.assertIn("task-oriented prompts", text)
        self.assertIn("GroundingDINO_SwinT_OGC.py", text)
        self.assertIn("groundingdino_swint_ogc.pth", text)
        self.assertIn("box_threshold=0.25", text)
        self.assertIn("text_threshold=0.20", text)
        self.assertIn("front-camera frame", text)
        self.assertIn("local_regions=True", text)
        self.assertIn("filter_grasps=True", text)
        self.assertIn("Single-camera CGN uses only `front_view`", text)
        self.assertIn("Two-camera fused RGB-D CGN uses `front_view` and `side_view`", text)
        self.assertIn("the fused segmented geometry is passed to the Contact-GraspNet proposal runner", text)
        self.assertIn("not only whether a grasp proposal exists", text)
    ...
    def test_single_current_report_is_the_documentation_entrypoint(self) -> None:
        ...
        self.assertIn("configs/results/cgn_official_depth_segmap_h100_20260508.json", text)
        self.assertIn("Contact-GraspNet Modular Pipeline", text)
        self.assertIn("Contact-GraspNet itself provides 6-DoF grasp proposals", text)
        self.assertIn("language grounding, two-view fusion, and robot execution wrapper", text)
        self.assertIn("target-specific trials", text)
        self.assertIn("task-oriented prompts", text)
        self.assertIn("GroundingDINO_SwinT_OGC.py", text)
        self.assertIn("groundingdino_swint_ogc.pth", text)
        self.assertIn("box_threshold=0.25", text)
        self.assertIn("text_threshold=0.20", text)
        self.assertIn("foreground-depth fallback", text)
        self.assertIn("front-camera frame", text)
        self.assertIn("pre-grasp, grasp, close, lift, and hold", text)
        self.assertIn("Single-camera Contact-GraspNet", text)
        self.assertIn("Two-camera fused RGB-D Contact-GraspNet", text)
        self.assertIn("back-projected with the front camera intrinsics", text)
        self.assertIn("transformed through the calibrated camera extrinsics", text)
        self.assertIn("concatenated into one segmented RGB-D geometry payload", text)
