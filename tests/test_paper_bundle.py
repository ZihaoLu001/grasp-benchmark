...
class PaperBundleTest(unittest.TestCase):
    ...
    def test_paper_bundle_writes_extended_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertTrue((output_dir / "teacher_summary_zh_clean.md").exists())
            ...
            teacher_text = (output_dir / "teacher_summary_zh_clean.md").read_text(encoding="utf-8")
            self.assertIn("仿真阶段总结", teacher_text)
            self.assertIn("shared benchmark + protocol audit", teacher_text)
            ...
            self.assertIn("## Instruction Robustness", report_text)
            self.assertIn("## Sim-to-Real Proxy Robustness", report_text)
            self.assertIn("## Phase 2 Pilot", report_text)
