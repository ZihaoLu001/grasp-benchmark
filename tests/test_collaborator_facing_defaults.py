...
class CollaboratorFacingDefaultsTest(unittest.TestCase):
    ...
    def test_readme_matches_current_pdf_task_sets(self) -> None:
        ...
        self.assertIn("CGN Official Depth+Segmap Proposal Appendix", text)
    ...
    def test_cgn_native_reference_appendix_is_backed_by_tracked_evidence(self) -> None:
        ...
        self.assertIn("not official Contact-GraspNet native performance", evidence["claim_boundary"])
    ...
    def test_cgn_official_depth_segmap_appendix_is_backed_by_tracked_evidence(self) -> None:
        ...
        self.assertIn("not an official end-to-end Contact-GraspNet native-system score", evidence["claim_boundary"])
        for job in evidence["slurm_jobs"]:
            self.assertEqual(job["state"], "COMPLETED")
            self.assertEqual(job["exit_code"], "0:0")
