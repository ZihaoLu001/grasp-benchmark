...
Task-Oriented Grasping Pilot note: the 24 trials are 8 mug-handle prompts, 8 avoid-inside-cup prompts, and 8 power-drill-handle prompts. This row should be read as an instruction-conditioned stress diagnostic under the shared lift-and-hold success rule. A target-specific Contact-GraspNet H100 rerun records `0 / 24`: all 24 trials localized a target and produced a non-empty mask, but Contact-GraspNet returned zero executable grasp proposals before planning. All four shards are present and duplicate scene IDs equal `0`.
...
| --- | --- | --- |
| Target localization | For target-specific trials, including language-conditioned and task-oriented prompts, GroundingDINO localizes the requested object class from the instruction or task specification. The configured detector uses `GroundingDINO_SwinT_OGC.py`, `groundingdino_swint_ogc.pth`, `box_threshold=0.25`, and `text_threshold=0.20`. Arbitrary-object trials use catalog labels and then a foreground-depth fallback. | target bounding box and detection score |
| Segmentation mask | The target box is converted into a depth-band mask using the rendered depth image. | binary target mask and one-segment object map |
...
For clarity, the two Contact-GraspNet camera settings correspond to two concrete data paths.

**Single-camera Contact-GraspNet.** The pipeline uses only `front_view`. For target-specific trials, GroundingDINO detects the requested target in the front RGB image; the front depth image is filtered inside the detected box to form a target mask; the masked front depth is back-projected with the front camera intrinsics `K` to obtain a target point cloud; Contact-GraspNet receives the front depth, `K`, RGB, and segmentation map through its checked `depth + K + segmap + RGB` proposal path; the selected 6-DoF grasp is then converted into the shared pre-grasp, grasp, close, lift, and hold action sequence.

**Two-camera fused RGB-D Contact-GraspNet.** The pipeline uses both `front_view` and `side_view`. First, the target mask is built on the front view as above. Second, the side view is processed with the same target-localization logic for language-conditioned trials, or a foreground-depth fallback for arbitrary grasping. Third, front depth plus the front mask is back-projected into a front-view point cloud, and side depth plus the side mask is back-projected into a side-view point cloud. Fourth, the side-view points are transformed through the calibrated camera extrinsics into the front-camera frame. Fifth, the front and transformed side point clouds are concatenated into one segmented RGB-D geometry payload and passed to the Contact-GraspNet proposal runner. The returned ranked grasps are executed by the same benchmark planner/controller as the single-camera setting.
