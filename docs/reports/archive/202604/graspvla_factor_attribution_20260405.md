...
- The latest formal shared-benchmark rerun is `14/15` for `GraspVLA` under `track_a_cal_v1`.
...
| `official subset -> Track A-Cal shared distribution` | `V4 -> V5` | This is no longer evidence of a catastrophic drop. The latest formal Track A-Cal rerun is `14/15`, so the current shared calibration scenes themselves are not the source of the old all-zero result. |
...
- The earlier **`Track A-Cal = 0/15` was mostly an implementation-era artifact**, not the stable result of the current shared benchmark. The current formal shared rerun shows `14/15`.
